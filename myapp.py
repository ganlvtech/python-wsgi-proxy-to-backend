import cgi
import os
import subprocess
import traceback
from tempfile import TemporaryFile

import wsgi_proxy


def get_server_program_path():
    if os.name == 'nt':
        path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'USERRES/server.exe')
    elif os.name == 'posix':
        path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'USERRES/server')
    else:
        path = None
    return path


def terminate_process(name):
    if os.name == 'nt':
        subprocess.call(['taskkill', '/f', '/im', str(name)])
    elif os.name == 'posix':
        subprocess.call(['killall', '-9', str(name)])


def teminate_server_process():
    path = get_server_program_path()
    name = os.path.basename(path)
    try:
        terminate_process(name)
    except Exception as e:
        traceback.print_exc()


def run_server_program(port=None):
    path = get_server_program_path()
    os.chmod(path, 0o755)
    args = [path]
    if port:
        args.append(str(port))
    try:
        proc = subprocess.Popen(args, close_fds=True)
        return proc
    except Exception as e:
        traceback.print_exc()


def handle_upload_get(environ, start_response):
    upload_password = os.environ.get('BKAPP_UPLOAD_PASSWORD')
    if not upload_password:
        content = u'Upload is not allowed.'
        content = content.encode()
        start_response('403 Forbidden', [
            ('Content-Type', 'text/plain; charset=UTF-8'),
            ('Content-Length', str(len(content))),
        ])
        return [content]

    content = u'''
<form method="post" enctype="multipart/form-data">
    <p><input type="password" name="password"></p>
    <p><input type="file" name="file"></p>
    <p><button type="submit">Upload</button></p>
</form>
'''
    content = content.encode()
    start_response('200 OK', [
        ('Content-Type', 'text/html; charset=UTF-8'),
        ('Content-Length', str(len(content))),
    ])
    return [content]


def handle_upload_post(environ, start_response):
    upload_password = os.environ.get('BKAPP_UPLOAD_PASSWORD')
    if not upload_password:
        content = u'Upload is not allowed.'
        content = content.encode()
        start_response('403 Forbidden', [
            ('Content-Type', 'text/plain; charset=UTF-8'),
            ('Content-Length', str(len(content))),
        ])
        return [content]

    try:
        fields = cgi.FieldStorage(fp=environ['wsgi.input'], environ=environ)
    except Exception as e:
        traceback.print_exc()
        content = e.message
        content = content.encode()
        start_response('500 Internal Server Error', [
            ('Content-Type', 'text/plain; charset=UTF-8'),
            ('Content-Length', str(len(content))),
        ])
        return [content]

    if 'password' not in fields:
        content = u'Password required!'
        start_response('401 Unauthorized', [
            ('Content-Type', 'text/plain; charset=UTF-8'),
            ('Content-Length', str(len(content))),
        ])
        return [content]

    password_field = fields['password']
    if password_field.value != upload_password:
        content = u'Password wrong!'
        content = content.encode()
        start_response('401 Unauthorized', [
            ('Content-Type', 'text/plain; charset=UTF-8'),
            ('Content-Length', str(len(content))),
        ])
        return [content]

    if 'file' not in fields:
        proc = run_server_program(8003)

        content = u'Server runs at pid {}.'.format(proc.pid)
        content = content.encode()
        start_response('200 OK', [
            ('Content-Type', 'text/plain; charset=UTF-8'),
            ('Content-Length', str(len(content))),
        ])
        return [content]
    
    file_item = fields['file']
    if not isinstance(file_item, cgi.FieldStorage):
        content = u'Field "file" is not a file or contains multiple files.'
        content = content.encode()
        start_response('400 Bad Request', [
            ('Content-Type', 'text/plain; charset=UTF-8'),
            ('Content-Length', str(len(content))),
        ])
        return [content]

    teminate_server_process()

    path = get_server_program_path()
    path_dir = os.path.dirname(path)
    if not os.path.exists(path_dir):
        os.makedirs(path_dir, mode=0o755)

    with open(path, 'wb') as f:
        f.write(file_item.file.read())

    run_server_program(8003)

    content = u'The file was uploaded successfully'
    content = content.encode()
    start_response('200 OK', [
        ('Content-Type', 'text/plain; charset=UTF-8'),
        ('Content-Length', str(len(content))),
    ])
    return [content]


def application(environ, start_response):
    method = environ['REQUEST_METHOD']
    path = environ['PATH_INFO']
    query = environ['QUERY_STRING']
    upload_query = os.environ.get('BKAPP_UPLOAD_QUERY')

    if method == 'GET' and path == '/upload' and query == upload_query:
        return handle_upload_get(environ, start_response)
    if method == 'POST' and path == '/upload' and query == upload_query:
        return handle_upload_post(environ, start_response)

    environ['HTTP_HOST'] = '127.0.0.1:8003'
    environ['wsgi.url_scheme'] = 'http'
    return wsgi_proxy.app(environ, start_response)
